...
def load_encoders():
    with open('business_encoder.pickle', 'rb') as b:
        business_encoder = pickle.load(b)

    with open('user_encoder.pickle', 'rb') as b:
...
def load_cache():
    try:
        with open('recommendation_cache.pickle', 'rb') as b:
...
def load_content_scores():
    with open('content_based_scores.pickle', 'rb') as b:
...
def load_data():
    ...
    collab_model.load_weights('model_weights.h5')
...
def get_photo(restaurant_id, df):
    ...
    try:
        ...
        filepath = f"https://res.cloudinary.com/dnsvyb8zc/image/upload/Home/{filename}.jpg"  # case-sensitive folder!
    except:
        filepath = "https://cdn.dribbble.com/users/1012566/screenshots/4187820/media/985748436085f06bb2bd63686ff491a5.jpg"
    print("Serving image:", filepath)
    return filepath
...
def load_categories():
    with open('search_category.pickle', 'rb') as b:
